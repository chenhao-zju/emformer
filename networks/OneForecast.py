...
class OneForecast(Module):
    ...
    def __init__(
        ...
        multimesh: bool = True,
        input_res: tuple = (128, 256),       # (120, 240),
        input_dim_grid_nodes: int = 5,
        input_dim_mesh_nodes: int = 3,
        input_dim_edges: int = 4,
        output_dim_grid_nodes: int = 5,
...
if __name__ == '__main__':
    ...
    # chkpt_dir = '/home/bingxing2/ailab/scxlab0094/hantao_bak/haochen/code/OneForecast-main/ckpt_oneforecast.tar'
    chkpt_dir = '/home/bingxing2/ailab/scxlab0094/hantao_bak/haochen/logs/oneforecast_79to20_69_0530/afno_backbone/1/training_checkpoints/ckpt.tar'

    net = OneForecast().to(device)

    checkpoint = torch.load(chkpt_dir, map_location='cpu')['model_state']
    new_checkpoint = {}
    for key, value in checkpoint.items():
        new_key = key[7:]
        if new_key=='encoder_embedder.grid_node_mlp.model.0.weight' or new_key=='finale.model.2.weight' or new_key=='finale.model.2.bias':
            continue
        else:
            new_checkpoint[new_key] = value

    msg = net.load_state_dict(new_checkpoint, strict=False)
    print(msg)

    input = torch.randn(1, 5, 128, 256).to(device)
    output = net(input)
